...
Seasonal weather forecast data is available in the `GET /api/weatherforecast?type=tenday` REST API endpoint (plus other query parameters) if the [ACAP-RCMAS REST APIs](/post-installation/acap-rcmas-api) are active.
