...
<Callout type="info" emoji="ℹ️">
ACAP 1.0's last stable version is Release/Tag version **9.5.6**, accessible in the **climate-services-webportal-v1** and **acap-v2** code repositories.
</Callout>
...
3. PDF Bulletins Generator
   - Generated bulletins matches the given JPEG layout and all content fits in one (1) page
4. Site Search
5. Public Pages
   - Home Page
   ...
   - Crop Recommendations Generator
      - Seasonal Climate Outlook and Advisory Recommendations (using month, crop stage/s, and rainfall condition as triggers)
      - 10-Day Farm Weather Outlook and Advisory Recommendations (using current date and crop stage as a trigger)
...
- Use `.env` variables with region-specific values in the front end, allowing easier regional static text replacement.
